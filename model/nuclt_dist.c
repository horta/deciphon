...
enum rc nuclt_dist_pack(struct nuclt_dist const *ndist, struct lip_file *file) {
  ...
  if (imm_nuclt_lprob_pack(&ndist->nucltp, file))
    return rc;
  if (imm_codon_marg_pack(&ndist->codonm, file))
    return rc;
  return RC_OK;
}
...
enum rc nuclt_dist_unpack(struct nuclt_dist *ndist, struct lip_file *file) {
  ...
  assert(size == 2);
  if (imm_nuclt_lprob_unpack(&ndist->nucltp, file))
    return rc;
  if (imm_codon_marg_unpack(&ndist->codonm, file))
    return rc;
  return RC_OK;
}
