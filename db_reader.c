                                     ...
                                     enum prof_typeid typeid)
{
  ...
  if (!lip_read_int(&db->file, &db->prof_typeid)) return RC_EFREAD;

  if (db->prof_typeid != typeid) return RC_EFDATA;
...
static enum rc unpack_header_profile_sizes(struct db_reader *db)
{
  ...
  if (type != LIP_1DARRAY_UINT32) return RC_EFDATA;

  db->profile_sizes = malloc(sizeof(*db->profile_sizes) * db->nprofiles);
  if (!db->profile_sizes) return RC_ENOMEM;
